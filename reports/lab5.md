1. 线程资源回收
Q: 在我们的多线程实现中，当主线程 (即 0 号线程) 退出时，视为整个进程退出， 此时需要结束该进程管理的所有线程并回收其资源。
  - 需要回收的资源有哪些？
  - 其他线程的 TaskControlBlock 可能在哪些位置被引用，分别是否需要回收，为什么？
A: 需要回收主线程的 TaskUserRes, 还需要回收所有线程的栈空间、跳板页等。其他线程的 TaskControlBlock 会在其进程容器和线程调度器中被引用，它们无需被回收，因为它们不属于当前进程或主线程管理。
...
```
A: 应当采取 Mutex2 的实现。Mutex1 实现的问题在于 waking\_task 被唤醒后不会进行 mutex\_inner.locked=true 的过程。因此当有 waking\_task 时应该进行平滑的切换，保持 mutex\_inner.locked=true。
